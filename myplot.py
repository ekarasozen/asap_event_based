...
def scals(t, tr, Xo, Xd, rho, alpha, Xp, abovethreshold, freq, outpath, fig, event_list, station_list, figname="scals"): 
    ...
    maxamp_r = 20          # these two lines just adjust the color scale
    maxamp_a = abs(alpha).max()/2           # these two lines just adjust the color scale
    maxamp_p = abs(Xp).max()/2           # these two lines just adjust the color scale
    maxamp_pa = abs(abovethreshold).max()/2           # these two lines just adjust the color scale
    minamp = 0
    tX, f = np.meshgrid(tr.times(), freq)
    ax11 = fig.add_axes([0.1, 0.85, 0.7, 0.1])
    ax111 = fig.add_axes([0.83, 0.85, 0.03, 0.1])
    ax12 = fig.add_axes([0.1, 0.70, 0.7, 0.1])
    ax121 = fig.add_axes([0.83, 0.70, 0.03, 0.1])
    ax13 = fig.add_axes([0.1, 0.55, 0.7, 0.1])
    ax131 = fig.add_axes([0.83, 0.55, 0.03, 0.1])
    ax14 = fig.add_axes([0.1, 0.40, 0.7, 0.1])
    ax141 = fig.add_axes([0.83, 0.40, 0.03, 0.1])
    ax15 = fig.add_axes([0.1, 0.25, 0.7, 0.1])
    ax151 = fig.add_axes([0.83, 0.25, 0.03, 0.1])
    ax16 = fig.add_axes([0.1, 0.05, 0.7, 0.1])
    ax161 = fig.add_axes([0.83, 0.05, 0.03, 0.1])
    img_o = ax11.pcolormesh(tX, f, np.abs(Xo), cmap=cm.hot, vmin=minamp, vmax=maxamp_o)
    img_d = ax12.pcolormesh(tX, f, np.abs(Xd), cmap=cm.hot, vmin=minamp, vmax=maxamp_d)
    img_r = ax13.pcolormesh(tX, f, np.abs(rho), cmap=cm.hot, vmin=minamp, vmax=maxamp_r)
    img_a = ax14.pcolormesh(tX, f, np.abs(alpha), cmap=cm.hot, vmin=minamp, vmax=maxamp_a)
    img_p = ax15.pcolormesh(tX, f, np.abs(Xp), cmap=cm.hot, vmin=minamp, vmax=maxamp_p)
    img_pa = ax16.pcolormesh(tX, f, np.abs(abovethreshold), cmap=cm.hot, vmin=minamp, vmax=maxamp_pa)
    ax11.set_title('origina')
    ax11.axes.xaxis.set_visible(False)
    ax12.set_title('degraded')
    ax12.axes.xaxis.set_visible(False)
    ax13.set_title('rho')
    ax13.axes.xaxis.set_visible(False)
    ax14.set_title('alpha')
    ax14.set_ylabel("frequency [Hz]")
    ax14.axes.xaxis.set_visible(False)
    ax15.set_title('processed')
    ax15.axes.xaxis.set_visible(False)
    ax16.set_title('XP_abovethreshold')
    ax16.axes.xaxis.set_visible(True)
    ax16.set_xlim(t[0], t[-1])
    ax16.set_ylim(freq[-1], freq[0])
    ax16.set_xlabel("time after %s [s]" % tr.stats.starttime)
    fig.colorbar(img_o, cax=ax111)
    fig.colorbar(img_d, cax=ax121)
    fig.colorbar(img_r, cax=ax131)
    fig.colorbar(img_a, cax=ax141)
    fig.colorbar(img_p, cax=ax151)
    fig.colorbar(img_pa, cax=ax161)
    fig.autofmt_xdate()    
    fig.savefig(os.path.join(outpath, event_list + '_'+ station_list + '_' + figname + '.png'))

def subtraction_performance(amp_Xo,amp_Xd,amp_Xp,freqs_d,picktime,tro,trd,trp,tr_SNR,tr_alpha,metrics,SNR,alpha,alpha0,ss_type,outpath,fig,event_list,station_list,phi="0",figname="subtraction_parameters"):
    '''
