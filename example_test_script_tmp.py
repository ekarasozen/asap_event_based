...
#--     fig2 = plot.all(t_d, trd, Xd, freq_d, IXd, fig2, event_list[e], figname="degraded")
    ...
    trn.trim(t0+25, t0+35) #this option might be better    # EDITED - NOTE THIS INCLUDES SIGNAL!
    #trn.detrend("linear")
    #trn.detrend("demean")
    ...
    amp_Xn = abs(Xn) 
#--    Xna = np.mean(Xn,axis=1)                   # MOVED INSIDE SPECTRAL SUBTRACTION
    trp = trd.copy() # 
    amp_Xp = ss.simple_subtraction_tmp(amp_Xd,amp_Xn,2)        # EDITED
    phase_Xd = np.angle(Xd) 
    Xp = amp_Xp*(np.exp(1.j*phase_Xd))
    #IXp = mlwt.icwt(Xp, trd) ###mlpy
    trp.data = wavelet.icwt(Xp, scales_d, dt, dj=0.05, wavelet='morlet') ###pycwt   # EDITED
#--    fig3 = plt.figure()
#--    fig3 = plot.all(t_d, trd, Xp, freq_d, IXp, fig3, event_list[e], figname="processed")
#--    fig4 = plt.figure()
...
# PLOT THE amp_Xd, amp_Xn, amp_Xp MATRICES
import matplotlib.cm as cm
fig, (ax1, ax2, ax3) = plt.subplots(3,1)
maxamp = abs(amp_Xo).max()/2           # these two lines just adjust the color scale
minamp = 0
td, fd = np.meshgrid(tro.times(), freq)
tn, fn = np.meshgrid(trn.times(), freq)
im1 = ax1.pcolormesh(td, fd, np.abs(amp_Xd), cmap=cm.hot, vmin=minamp, vmax=maxamp)
im2 = ax2.pcolormesh(tn, fn, np.abs(amp_Xn), cmap=cm.hot, vmin=minamp, vmax=maxamp)
im3 = ax3.pcolormesh(td, fd, np.abs(amp_Xp), cmap=cm.hot, vmin=minamp, vmax=maxamp)
ax1.set_ylabel('frequency (Hz)')
ax2.set_ylabel('frequency (Hz)')
ax3.set_ylabel('frequency (Hz)')
ax3.set_xlabel('time (s)')
ax1.set_title('amp_Xd     amp_Xn     amp_Xp')
plt.show()
